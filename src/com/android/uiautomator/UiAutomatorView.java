...
public class UiAutomatorView extends Composite {
    ...
    private TableViewer checkmTableViewer;
    private BasicTreeNode editNode;
    private int editIndex;
    private Random random = new Random();
    private List<BasicTreeNode> commitList = new ArrayList<BasicTreeNode>();
...
//        mScreenshotComposite.setBackgroundImage(ImageHelper.loadImageDescriptorFromResource("images/wx.png").createImage());
...
//                    dlg.updateSelXpathInfo(xpath, desp, name);
        ...
        lowerRightBase.setLayout(new FillLayout(SWT.VERTICAL));
        SashForm lowerRightBaseSash = new SashForm(lowerRightBase, SWT.VERTICAL);
        
        Composite c = new Composite(lowerRightBaseSash, SWT.BORDER);
        c.setLayout(new FillLayout(SWT.HORIZONTAL));
        Group grpNodeDetail = new Group(c, SWT.NONE);
        ...
        final Table table = mTableViewer.getTable();
    ...
    class TableColumnContentProvider implements IStructuredContentProvider {    
		...
		public Object[] getElements(Object inputElement) {
         	if (inputElement instanceof Collection<?>) {    
                 // 筛选出没有删除的列   
                 List<BasicTreeNode> nonDelCols = new LinkedList<BasicTreeNode>();    
                 for (BasicTreeNode node : (List<BasicTreeNode>) inputElement) {    
                     nonDelCols.add(node);    
                 }    
    ...
    private class AttributeTableEditingSupport2 extends EditingSupport {

        private TableViewer mViewer;

        public AttributeTableEditingSupport2(TableViewer viewer) {
            super(viewer);
            mViewer = viewer;
        }
        ...
        protected CellEditor getCellEditor(Object arg0) {
        	TextCellEditor cellEditor = new TextCellEditor(mViewer.getTable());
            return cellEditor;
        }

        @Override
        protected Object getValue(Object o) {
            return ((UiNode) o).getAttribute("myname")==null?"":((UiNode) o).getAttribute("myname");
        }

        @Override
        protected void setValue(Object o, Object value) {
        	if(editNode!=null){
            	((UiNode)editNode).addAtrribute("myname", value.toString());
            	Const.checkList.set(editIndex, editNode);
            	myloadAttributeTable();
        	}
        }
    }
	 ...
	 * 加载已经选中的node
	 */
    public void myloadAttributeTable() {
        checkmTableViewer.setInput(Const.checkList);
    }
